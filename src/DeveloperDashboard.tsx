...
const DeveloperDashboard = ({ onLogout }: { onLogout?: () => void }) => {
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', background: '#020617', color: '#fff', overflow: 'hidden' }}>
      ...
      <main style={{ flex: 1, overflowY: 'auto', padding: '3rem 4rem', background: 'radial-gradient(circle at 50% 0%, rgba(99, 102, 241, 0.05) 0%, transparent 50%)' }}>
        {activeSection === 'blocked' && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', textAlign: 'center' }}>
            ...
            <h2 style={{ fontSize: '2rem', fontWeight: 800, marginBottom: '1rem' }}>Developer Verification Required</h2>
        ...
        {activeSection === 'overview' && (
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1.2fr) minmax(0, 0.8fr)', gap: '3rem' }}>
              <div>
                    ...
                    <div style={{ background: '#0a0f1e', border: '1px solid var(--border)', borderRadius: '24px', overflow: 'hidden' }}>
                       {[
                         ...
                         { method: 'GET', endpoint: '/v1/users/me', status: 200, time: '1m ago', latency: '35ms' },
                          ...
                          <div key={i} style={{ display: 'flex', padding: '1rem 1.5rem', borderBottom: i === 3 ? 'none' : '1px solid rgba(255,255,255,0.03)', fontSize: '0.8rem', alignItems: 'center' }}>
              ...
              <div>
                 ...
                 <section>
                    ...
                    {userData?.wallets?.slice(0, 3).map((w: any) => (
                      <div key={w.id} style={{ padding: '1.25rem', background: 'rgba(255,255,255,0.02)', border: '1px solid var(--border)', borderRadius: '20px', marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '1rem' }}>
                         ...
                         <div style={{ flex: 1 }}>
                            <div style={{ fontSize: '0.9rem', fontWeight: 800 }}>{w.currency} Rail</div>
                            <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)', fontFamily: 'monospace' }}>{JSON.parse(w.metadata || '{}').iban?.slice(0, 16)}...</div>
                         </div>
                         <div style={{ fontWeight: 800, color: 'var(--primary)' }}>{parseFloat(w.balance).toFixed(2)}</div>
                      </div>
                    ))}
                 </section>
              </div>
